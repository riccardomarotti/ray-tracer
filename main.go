...
func main() {
	...
	sphere := Sphere{Identity().Translate(-0.5, 1, 0.5), sphereMaterial}
	...
	sphere2 := Sphere{Identity().Translate(1.5, 0.5, -0.5).Scale(0.5, 0.5, 0.5), sphere2Material}
	...
	sphere3 := Sphere{Identity().Translate(-1.5, 0.33, -0.75).Scale(0.33, 0.33, 0.33), sphere3Material}

	floorMaterial := DefaultMaterial()
	floorMaterial.color = Color{1, 0.9, 0.9}
	floorMaterial.specular = 0
	floor := Sphere{Identity().Scale(10, 0.01, 10), floorMaterial}

	leftWall := Sphere{Identity().Translate(0, 0, 5).RotateY(-math.Pi/4).RotateX(math.Pi/2).Scale(10, 0.01, 10), floorMaterial}
	rightWall := Sphere{Identity().Translate(0, 0, 5).RotateY(math.Pi/4).RotateX(math.Pi/2).Scale(10, 0.01, 10), floorMaterial}
